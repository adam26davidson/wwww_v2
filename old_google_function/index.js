...
const co = require('co');
const url = "mongodb+srv://adam26davidson:groovyman77@cluster0.axrdl.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
...
exports.wwww = (req, res) => {
  co(function*() {
    ...
    if (body.type == "enter-waiting-room") {
      ...
      let activeCodesArray = yield client.db('wwww').collection('active-room-codes').find().toArray();
      let activeCodes = activeCodesArray[0].codes;
      if (activeCodes.includes(body.code)) {
        const query = {code: body.code}
        let gameState = yield client.db('wwww').collection('active-games').findOne(query);
        if (gameState.gameStage == "waiting-room") {
          ...
          yield client.db('wwww').collection('active-games').updateOne(query, update);
          responseData = {
      ...
      yield client.db('wwww').collection('active-room-codes').updateOne(query, update);
      ...
      if (activeCodes.includes(body.code)) {
        ...
        if( gameState.responses.length == gameState.numPlayers - 1) {
          update = {
            ...
            $set: {gameStage: "all-players-waiting", players: game.state.players}
          }
        } else {
          update = {
            $push: {responses: body.response},
      ...
      if (activeCodes.includes(body.code)) {
        const query = {code: body.code}
        let gameState = yield client.db('wwww').collection('active-games').findOne(query);
        if (gameState.gameStage == 'all-players-waiting' || gameState.gameStage == 'sentences-created') {
          let game = new Game();
          game.state = gameState;
